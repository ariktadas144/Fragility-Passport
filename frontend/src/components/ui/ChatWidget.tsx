...
export default function ChatWidget() {
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        { role: "assistant", text: `Could not reach the assistant (${String(err)}).` },
      ]);
    } finally {
      setSending(false);
          ...
          <CardContent ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-background">
            ...
            {messages.map((m, i) => (
              <div key={i} className={cn("flex", m.role === "user" ? "justify-end" : "justify-start")}>
                <div
                  className={cn(
                    "px-4 py-3 rounded-2xl max-w-[85%] text-sm border shadow-sm whitespace-pre-wrap",
                    m.role === "user"
                      ? "bg-accent-indigo text-white rounded-tr-sm border-indigo-500"
                      : "bg-zinc-900 text-zinc-300 rounded-tl-sm border-zinc-800"
                  )}
                >
                  {m.text}
                  {m.source && (
                    <span className="block mt-1 text-[10px] uppercase tracking-wider text-zinc-500">
                      {m.source === "local" ? "from event log" : m.source === "llm" ? "AI reasoning" : "not tracked"}
                    </span>
                  )}
                </div>
              </div>
            ...
            {sending && (
              <div className="flex justify-start">
                <div className="bg-zinc-900 text-zinc-500 px-4 py-3 rounded-2xl rounded-tl-sm text-sm border border-zinc-800">
                  Thinking…
                </div>
              </div>
            )}
          ...
          <div className="p-3 bg-card-bg border-t border-border-subtle shrink-0">
            <div className="relative">
              ...
              <button
                onClick={send}
                disabled={sending}
                className="absolute right-1.5 top-1.5 h-7 w-7 bg-accent-indigo text-white rounded-full flex items-center justify-center hover:bg-indigo-500 transition-colors disabled:opacity-50"
              >
